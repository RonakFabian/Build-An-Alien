...
public class CharacterPlacement : MonoBehaviour
{
  ...
  [Header("Objects:")] public Animator leftPanelAnimator;
  ...
  public GameObject battleWheel;

  [Space(5)] public GameObject characterPanel;
  ...
  [Space(5)] public Image eraseButton;
  ...
  [Space(5)] public Dropdown speedSetting;

  [Space(5)] public Transform gridPanel;
  public Transform gridArrow;

  [Header("Troops:")] public List<Troop> troops;
  ...
  void Awake()
  {
    ...
    {
      // levelInfo.text = "Level " + (PlayerPrefs.GetInt("level") + 1) + " - " +
      //                  levelData.levels[PlayerPrefs.GetInt("level")].scene;
    }
  }
  ...
  void Lose()
  {
    ...
    defeatPanel.GetComponent<Animator>().SetTrigger("defeat");
  }
  ...
  void Win()
  {
    ...
    winPanel.GetComponent<Animator>().SetTrigger("victory");
  }

  void Update()
  {
    battleWheel.SetActive(false);
    //if the battle has started
    if (battleStarted)
    {
      battleWheel.SetActive(true);
    ...
    if (currentDemoCharacter)
    {
      ...
      if (!erasing)
      {
        ...
        if (unitsInRange(position) != null || position.x < 0 || troops[selected].troopCosts > coins ||
        ...
        foreach (Renderer renderer in currentDemoCharacter.transform.Find("Indicator(Clone)")
      ...
      if (currentDemoCharacter.activeSelf && currentDemoCharacter.GetComponent<Animator>() &&
          currentDemoCharacter.GetComponent<Animator>().GetBool("Start") != false)
      {
  ...
  bool withinGrid(Vector3 position)
  {
    ...
    if (position.x > gridCenter.x + gridSize || position.x < gridCenter.x - gridSize ||
        position.z < gridCenter.z - gridSize || position.z > gridCenter.z + gridSize)
      return false;
  ...
  bool canPlace(Vector3 position, bool placingGridCell)
  {
    ...
    if (!placingGridCell && (EventSystem.current.IsPointerOverGameObject() ||
                             Vector3.Distance(Camera.main.transform.position, position) > levelData.placeRange))
      return false;
  ...
  {
    position = new Vector3(Mathf.RoundToInt(position.x) - gridCenter.x, position.y,
        Mathf.RoundToInt(position.z) - gridCenter.z);
    int index = 0;
  ...
  public void eraseUnit(Vector3 position, bool clearing, bool erasingGridCell)
  {
    ...
    if (unit != null && unit.name.Length - 7 > 0 && unit.gameObject.tag != "Enemy" &&
        (!EventSystem.current.IsPointerOverGameObject() || clearing || erasingGridCell))
    {
      if (!clearing)
  ...
  public GameObject unitsInRange(Vector3 position)
  {
    ...
    {
      if (Vector3.Distance(unit.gameObject.transform.position, position) < levelData.checkRange &&
          unit.gameObject != currentDemoCharacter)
    ...
    foreach (Renderer renderer in currentDemoCharacter.GetComponentsInChildren<Renderer>())
    {
      foreach (Material material in renderer.materials)
      {
        ...
        material.color = new Color(material.color.r, material.color.g, material.color.b,
            levelData.demoCharacterAlpha * colorStrength);
      }
    }
  ...
  public Vector3 getPosition()
  {
    ...
    if (Physics.Raycast(ray, out hit) && hit.collider != null && !EventSystem.current.IsPointerOverGameObject() &&
        hit.collider.gameObject.tag == "Battle ground")
    {
  ...
  IEnumerator EndEvents()
  {
    ...
    foreach (var unit in u)
    {
      if (unit)
      {
        unit.GetComponent<Animator>().SetBool("Idle", true);
        unit.GetComponent<AudioSource>().enabled = false;

        unit.PlayVictoryAnim();
      }
    }
  ...
  public void ToggleSpeed(GameObject g)
  {
    normalSpeed = !normalSpeed;
    if (normalSpeed)
    {
      Time.timeScale = 1;
      g.GetComponent<Button>().image.color = Color.white;
    }
    else
    {
      Time.timeScale = 0.5f;
