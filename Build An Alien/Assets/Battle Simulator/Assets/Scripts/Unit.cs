...
public class Unit : MonoBehaviour
{
    ...
    [HideInInspector] public bool spread;
    ...
    [HideInInspector] private float startLives;
    ...
    private bool dead;
    private float dt;
    public float rateofAttack=0.43f;

    private MeleeEnemyMesh meleeEnemyMesh;
    private UnitMeshManager unitMeshManager;
    ...
    void Start()
    {
        ...
        meleeEnemyMesh = GetComponent<MeleeEnemyMesh>();
        //  dt = rateofAttack;
    ...
    void FixedUpdate()
    {
        ...
        // //play dusteffect when running and stop it when the character is not running
         if(dustEffect && hAnimator.GetBool("Attacking") == false && !dustEffect.isPlaying)
         	dustEffect.Play();

         if(dustEffect && dustEffect.isPlaying && hAnimator.GetBool("Attacking") == true)
         	dustEffect.Stop();
        ...
        else
        {
            ...
            if (Vector3.Distance(currentTarget.position, transform.position) <= agent.stoppingDistance)
            {
                ...
                transform.LookAt(currentTargetPosition);
                  if(hAnimator)
                hAnimator.SetBool("Attacking", true);
                if(tAnimator)
                    tAnimator.SetBool("Attacking", true);
                if(lAnimator)
                    lAnimator.SetBool("Attacking", true);
            ...
            if (hAnimator.GetBool("Attacking") &&
                Vector3.Distance(currentTarget.position, transform.position) > agent.stoppingDistance)
            {
                hAnimator.SetBool("Attacking", false);
                tAnimator.SetBool("Attacking", false);
                lAnimator.SetBool("Attacking", false);
                ...
                for (int i = 0; i < enemies.Length; i++)
                {
                    ...
                    foreach (GameObject potentialTarget in availableEnemies)
                    {
                        ...
                        if (Vector3.Distance(transform.position, potentialTarget.transform.position) <
                            closestDistance && potentialTarget != null)
                        {
    ...
    public Vector3 getRandomPosition(WalkArea area)
    {
        ...
        Vector3 rayStart = new Vector3(center.x + Random.Range(-bounds.x / 2f, bounds.x / 2f), yRay,
            center.z + Random.Range(-bounds.z / 2f, bounds.z / 2f));
    ...
    public IEnumerator die()
    {
       ...
       if (go)
       {
           if (meleeEnemyMesh)
           {
               go.GetComponent<Ragdoll>().SetEnemyMesh();
           }

           if (unitMeshManager)
           {
               go.GetComponent<Ragdoll>().SetAllyMesh();
           }
    ...
    {
        hAnimator.SetTrigger("Victory");
        lAnimator.SetTrigger("Victory");
        tAnimator.SetTrigger("Victory");
        agent.enabled=false;
